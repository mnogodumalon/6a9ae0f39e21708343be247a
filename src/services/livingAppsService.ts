...
import type { Gaeste, Zimmer, Zusatzleistungen, Buchungen, Rechnungen, CreateGaeste, CreateZimmer, CreateZusatzleistungen, CreateBuchungen, CreateRechnungen } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getZusatzleistungen(): Promise<Zusatzleistungen[]> {
    ...
    return hydrateRecords(records, 'zusatzleistungen');
  }
  static async queryZusatzleistungen(q: RecordQuery = {}): Promise<Zusatzleistungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ZUSATZLEISTUNGEN}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Zusatzleistungen[];
  ...
  static async countZusatzleistungen(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.ZUSATZLEISTUNGEN}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  static async getZusatzleistungenEntry(id: string): Promise<Zusatzleistungen | undefined> {
    ...
    return hydrateRecords([record], 'zusatzleistungen')[0];
  }
