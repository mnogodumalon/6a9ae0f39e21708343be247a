...
import type { Rechnungen, Buchungen, LookupValue } from '@/types/app';
...
export type RechnungenDialogDefaults = Omit<Rechnungen['fields'], 'zahlungsstatus'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  zahlungsstatus: LOOKUP_OPTIONS['rechnungen']?.['zahlungsstatus'] ?? [],
};
...
export function RechnungenDialog({ open, onClose, onSubmit, defaultValues, recordId, buchungenList, enablePhotoScan = true, enablePhotoLocation = true }: RechnungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['buchung', 'betrag'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "buchung": string | null, // Display name from Buchungen (see <available-records>)\n  "betrag": number | null, // Betrag (€)\n  "rechnungsdatum": string | null, // YYYY-MM-DD\n  "faellig_am": string | null, // YYYY-MM-DD\n  "zahlungsstatus": LookupValue | null, // Zahlungsstatus (select one key: "offen" | "teilweise_bezahlt" | "bezahlt") mapping: offen=Offen, teilweise_bezahlt=Teilweise bezahlt, bezahlt=Bezahlt\n  "zahlungseingang": string | null, // YYYY-MM-DDTHH:MM\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["buchung"]);
        ...
        if (buchungName) {
          const buchungMatch = buchungenList.find(r => matchName(buchungName!, [String(r.fields.bemerkung ?? '')]));
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'buchung': (
      <div key="buchung" className="space-y-1.5">
        <Label htmlFor="buchung">{fieldLabel('rechnungen', 'buchung')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="buchung"
        ...
        />
        {showErrors && !fields.buchung && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('rechnungen', 'buchung')}</p>
        )}
      </div>
    ),
    'betrag': (
      <div key="betrag" className="space-y-1.5">
        <Label htmlFor="betrag">{fieldLabel('rechnungen', 'betrag')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="betrag"
          type="number"
          inputMode="decimal"
        ...
        {showErrors && !fields.betrag && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('rechnungen', 'betrag')}</p>
        )}
      </div>
    ),
    'rechnungsdatum': (
      <div key="rechnungsdatum" className="space-y-1.5">
        <Label htmlFor="rechnungsdatum">{fieldLabel('rechnungen', 'rechnungsdatum')}</Label>
        <DatePicker
          id="rechnungsdatum"
          placeholder="Datum ausstellen"
          mode="date"
    ...
    'faellig_am': (
      <div key="faellig_am" className="space-y-1.5">
        <Label htmlFor="faellig_am">{fieldLabel('rechnungen', 'faellig_am')}</Label>
        <DatePicker
          id="faellig_am"
    ...
    'zahlungsstatus': (
      <div key="zahlungsstatus" className="space-y-1.5">
        <Label htmlFor="zahlungsstatus">{fieldLabel('rechnungen', 'zahlungsstatus')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('rechnungen', 'zahlungsstatus', 'offen') ?? 'Offen'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('rechnungen', 'zahlungsstatus', 'teilweise_bezahlt') ?? 'Teilweise bezahlt'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('rechnungen', 'zahlungsstatus', 'bezahlt') ?? 'Bezahlt'}
          </button>
        </div>
      </div>
    ),
    'zahlungseingang': (
      <div key="zahlungseingang" className="space-y-1.5">
        ...
        <DatePicker
          id="zahlungseingang"
          placeholder="Wann ist Geld eingegangen?"
          mode="datetime"
  ...
  const FIELD_LABELS: Record<string, string> = {"buchung": "Buchung", "betrag": "Betrag (€)", "rechnungsdatum": "Rechnungsdatum", "faellig_am": "Fällig am", "zahlungsstatus": "Zahlungsstatus", "zahlungseingang": "Zahlungseingang"};
  const CURRENCY_KEYS = new Set<string>(["betrag"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"buchung": {"gast": "Gast", "zimmer": "Zimmer", "anreise": "Anreise", "abreise": "Abreise", "status": "Status", "personen": "Personen", "begleitperson": "Begleitperson", "zusatzleistungen_buchung": "Zusatzleistungen", "bemerkung": "Bemerkung", "beleg": "Beleg"}};
    ...
    {createBuchungenOpen && (
      <BuchungenDialog
        ...
        defaultValues={createBuchungenInitial
          ? ({ bemerkung: createBuchungenInitial } as any)
          : undefined}
        gaesteList={[]}
        zimmerList={[]}
