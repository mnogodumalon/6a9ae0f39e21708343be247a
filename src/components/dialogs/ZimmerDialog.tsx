...
export type ZimmerDialogDefaults = Omit<Zimmer['fields'], 'kategorie'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  kategorie: LOOKUP_OPTIONS['zimmer']?.['kategorie'] ?? [],
};
...
export function ZimmerDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: ZimmerDialogProps) {
  ...
  const REQUIRED_FIELDS = ['bezeichnung', 'kategorie'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "bezeichnung": string | null, // Bezeichnung\n  "kategorie": LookupValue | null, // Kategorie (select one key: "einzelzimmer" | "doppelzimmer" | "suite") mapping: einzelzimmer=Einzelzimmer, doppelzimmer=Doppelzimmer, suite=Suite\n  "preis_pro_nacht": number | null, // Preis pro Nacht (€)\n  "etage": number | null, // Etage\n  "balkon": boolean | null, // Balkon\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'bezeichnung': (
      <div key="bezeichnung" className="space-y-1.5">
        <Label htmlFor="bezeichnung">{fieldLabel('zimmer', 'bezeichnung')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="bezeichnung"
          placeholder="z. B. Z-101 oder Schöne Aussicht"
          value={fields.bezeichnung ?? ''}
          onChange={e => setFields(f => ({ ...f, bezeichnung: e.target.value }))}
          required
        />
        {showErrors && !fields.bezeichnung && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('zimmer', 'bezeichnung')}</p>
        )}
      </div>
    ),
    'kategorie': (
      <div key="kategorie" className="space-y-1.5">
        <Label htmlFor="kategorie">{fieldLabel('zimmer', 'kategorie')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'einzelzimmer'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'einzelzimmer' ? undefined : 'einzelzimmer') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'einzelzimmer'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('zimmer', 'kategorie', 'einzelzimmer') ?? 'Einzelzimmer'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'doppelzimmer'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'doppelzimmer' ? undefined : 'doppelzimmer') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'doppelzimmer'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('zimmer', 'kategorie', 'doppelzimmer') ?? 'Doppelzimmer'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'suite'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'suite' ? undefined : 'suite') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'suite'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('zimmer', 'kategorie', 'suite') ?? 'Suite'}
          </button>
        </div>
        {showErrors && !fields.kategorie && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('zimmer', 'kategorie')}</p>
        )}
      </div>
    ),
    'preis_pro_nacht': (
      <div key="preis_pro_nacht" className="space-y-1.5">
        <Label htmlFor="preis_pro_nacht">{fieldLabel('zimmer', 'preis_pro_nacht')}</Label>
        <Input
          id="preis_pro_nacht"
          type="number"
          inputMode="decimal"
    ...
    'etage': (
      <div key="etage" className="space-y-1.5">
        <Label htmlFor="etage">{fieldLabel('zimmer', 'etage')}</Label>
        <Input
          id="etage"
          type="number"
          inputMode="decimal"
          step="any"
          {...numberInputProps(formEnhancements, 'etage')}
          placeholder="z. B. 2"
          value={fields.etage !== undefined ? fields.etage : (computedValues['etage'] ?? '')}
          onChange={e => setFields(f => ({ ...f, etage: clampNumberValue(formEnhancements, 'etage', e.target.value) }))}
        />
      </div>
    ),
    'balkon': (
      <div key="balkon" className="space-y-1.5">
        <Label htmlFor="balkon">{fieldLabel('zimmer', 'balkon')}</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="balkon"
            checked={!!fields.balkon}
            onCheckedChange={(v) => setFields(f => ({ ...f, balkon: !!v }))}
          />
          <Label htmlFor="balkon" className="font-normal">{fieldLabel('zimmer', 'balkon')}</Label>
        </div>
      </div>
    ),
    'foto': (
  ...
  const FIELD_LABELS: Record<string, string> = {"bezeichnung": "Bezeichnung", "kategorie": "Kategorie", "preis_pro_nacht": "Preis pro Nacht (€)", "etage": "Etage", "balkon": "Balkon", "foto": "Foto"};
