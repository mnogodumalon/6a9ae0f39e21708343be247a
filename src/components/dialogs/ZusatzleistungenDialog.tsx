...
export function ZusatzleistungenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: ZusatzleistungenDialogProps) {
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('zusatzleistungen') })
    : t('new_entity', { entity: appLabel('zusatzleistungen') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'name': (
      <div key="name" className="space-y-1.5">
        <Label htmlFor="name">{fieldLabel('zusatzleistungen', 'name')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="name"
          placeholder="z. B. Frühstück, Spa, Parkplatz"
          value={fields.name ?? ''}
          onChange={e => setFields(f => ({ ...f, name: e.target.value }))}
          required
        />
        {showErrors && !fields.name && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('zusatzleistungen', 'name')}</p>
        )}
      </div>
    ),
    'preis': (
      <div key="preis" className="space-y-1.5">
        <Label htmlFor="preis">{fieldLabel('zusatzleistungen', 'preis')}</Label>
        <Input
          id="preis"
          type="number"
          inputMode="decimal"
    ...
    'aktiv': (
      <div key="aktiv" className="space-y-1.5">
        <Label htmlFor="aktiv">{fieldLabel('zusatzleistungen', 'aktiv')}</Label>
        <div className="flex items-center gap-2 pt-1">
          ...
          />
          <Label htmlFor="aktiv" className="font-normal">{fieldLabel('zusatzleistungen', 'aktiv')}</Label>
        </div>
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"name": "Name", "preis": "Preis (€)", "aktiv": "Aktiv"};
